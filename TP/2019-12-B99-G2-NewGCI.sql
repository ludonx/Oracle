...
-- Date de cr�ation :  Novembre 2019
...
LIMIT
SESSIONS_PER_USER          UNLIMITED --d�finir le nombre maximal de sessions qu'un utilisateur pourra ouvrir.
CPU_PER_SESSION            UNLIMITED --d�finir le temps de processeur maximum en centi�mes de secondes qu'une session pourra utiliser.
CPU_PER_CALL               3000      --d�finir le temps de processeur maximum en centi�mes de secondes qu'une execution de requ�te ou la recup�ration d'une requ�te (FETCH) pourra utiliser.
CONNECT_TIME               45        --d�finir le temps en minutes pour la dur�e de connexion maximale d'une session.
LOGICAL_READS_PER_SESSION  DEFAULT   --d�finir le nombre maximal de blocs lus durant une session.
LOGICAL_READS_PER_CALL     1000      --d�finir le nombre maximal de blocs lus durant un "appel serveur"
PRIVATE_SGA                15K       --d�finir la taille en Kbytes ou MBytes que pourra utiliser une session.
COMPOSITE_LIMIT            5000000;  --d�finir le co�t total des limitations autoris�e pour une session.

-- Assigner un profil � un user
-- @ludo ALTER USER userr PROFILE app_user;

-- Cr�er un r�le et l'alimenter par des privil�ges
-- @ludo CREATE ROLE roleclients ;
-- @ludo GRANT SELECT, INSERT, UPDATE, DELETE ON CLIENTS2 TO roleclients ;

-- Assigner le r�le � un user
...
-- Cr�er des commentaires pour les tables:
...
COMMENT ON table COMMANDES 	IS 'Cette table regroupe les commandes pass�e par les clients';
COMMENT ON table DETAILCOM	IS 'Cette table regroupe les articles de chaque commandes avec les quantit�es command�es';

SELECT  table_name, comments FROM user_tab_comments;

-- Cr�er des commentaires pour les colonnes:
COMMENT ON column CLIENTS.CODCLI IS 'Code du client (Unique par client)';
COMMENT ON column CLIENTS.CIVCLI IS 'Civilit� du client';
COMMENT ON column CLIENTS.NOMCLI IS 'Nom du Client';
COMMENT ON column CLIENTS.PRENCLI IS 'Pr�nom du client';
COMMENT ON column CLIENTS.CATCLI IS 'Cat�gorie du client';
...
--=========================================================================
-- Nouvelles donn�es pour les syst�mes de recommandation
INSERT INTO COMMANDES (NUMCOM, CODCLI, DATCOM)
...
-- >>>>>>>>>>>>>>>>>>>>>>>>>> -- R�sultat g�n�r�:
/*
Visualisation des articles ins�r�s dans ARTICLES
=========================================================================
...
--=================================================================================================
--=================================================================================================
--= Manipulation intelligente des donn�es : D�pendances Fonctionnelles   =============== DEBUT ====
--=================================================================================================
--=================================================================================================
--=================================================================================================
--=================================================================================================
...
--=================================================================================================
--=================================================================================================
--= Manipulation intelligente des donn�es : D�pendances Fonctionnelles   =============== FIN ======
--=================================================================================================
--=================================================================================================
--=================================================================================================
--=================================================================================================
...
-- Idem pour les fonctions � quelques exceptions pr�s (CREATE ...)
SET SERVEROUTPUT ON
...
rem
rem *************************************************************************************
rem procedure d �limination des doubles exacts sur plusieurs attributs
...
-- Les performances ! Le temps de r�ponse sur les gros volumes de donn�es les BIG DATA !

-- Beaucoup d'autres BIG PROBLEM se posent :
-- Le choix des attributs cl�s qui servent pour l'�limination (le MATCH) !

-- Beaucoup d'autres BIG PROBLEM se posent :
-- Que faire des attributs NON cl�s qui ne servent pour l'�limination (le MERGE) !
...
-- ======================================================
-- ======================================================
REM -- Professeur : Mr M. F. Boufar�s
REM -- Big Data, Bases de Donn�es Avanc�es, Entrep�ts de donn�es
-- ======================================================
/*
-- Algo Data Deduplication + (DD+) [M. F. Boufar�s Octobre 2018]
...
--=================================================================================================
--=================================================================================================
--= Manipulation intelligente des donn�es : Elimination des doubles et des similaires   ==== FIN ==
--=================================================================================================
--=================================================================================================
--=================================================================================================
--=================================================================================================
...
--=================================================================================================
--=================================================================================================
--= Manipulation intelligente des donn�es : Qu'est-ce un syst�me de recommandation  ====== DEBUT ==
--=================================================================================================
--=================================================================================================
--=================================================================================================
--=================================================================================================
...
/*
FB.6.4.1. Construisez la vue � Vi � qui contient les articles command�s par un client donn� � Ci �
(pour une p�riode donn�e, exemple le mois de septembre d'une ann�e telle que 2018, 2019...).
G�n�ralisation dans une proc�dure PL/SQL !

FB.6.4.2. Construisez la vue � Vij � qui contient les articles command�s � la fois
...
On pourra g�n�raliser pour l'ensemble des amis de � Ci � (Clustering...!)
...
--=================================================================================================
--=================================================================================================
-- MFB FFF +++++++++++++++ solution avant premi�re +++++++++++ ****** === ****** +++++++++++++++===
--=================================================================================================
--=================================================================================================
--=================================================================================================

-- Les articles command�s par les clients !
-- On pourrait ne s'int�resser qu'� certains clients et certains articles
-- (c-a-d : Certains CODCLI des clients ; Certains REFART des articles...)

-- On pourrait ne s'int�resser qu'� certaines commandes
...
-- Les articles command�s par le client C001.
CREATE OR REPLACE VIEW V_C001 AS
SELECT * FROM V_CLIARTQTE
WHERE CLIENT = 'C001';

-- Les articles command�s par le client C002.
CREATE OR REPLACE VIEW V_C002 AS
SELECT * FROM V_CLIARTQTE
WHERE CLIENT = 'C002';
-- Les articles command�s par le client C003.
CREATE OR REPLACE VIEW V_C003 AS
SELECT * FROM V_CLIARTQTE
WHERE CLIENT = 'C003';
-- Etc...
-- Les articles command�s par le client C006.
CREATE OR REPLACE VIEW V_C006 AS
SELECT * FROM V_CLIARTQTE
WHERE CLIENT = 'C006';
-- Etc...
-- Les articles command�s, � la fois, par les clients C001 et C002.
-- 1�re Solution : Les articles command�s, � la fois, par les clients C001 et C002.
...
-- Remarque : Que faut-il faire pour les articles qui sont command�s par l'un mais pas par l'autre?

-- Les articles command�s, � la fois, par les clients C001 et C003.
-- 2�me Solution : Les articles command�s, � la fois, par les clients C001 et C003.
...
MINUS
SELECT ARTICLE FROM V_C002;
-- On recommande (on propose) alors � C001 les articles de C002 qu''il n''a pas command�s :
SELECT ARTICLE FROM V_C002
MINUS
SELECT ARTICLE FROM V_C001;
...
as
    ...
    nbArticle NUMBER := 0;
BEGIN
    -- Suppresion et Cr�ation de la table qui va contenir les articles qu'ont les clients en commun
    ...
    -- Parcours sur tous les clients pour creer une vue Vij avec i et j des clients diff�rents
    ...
    -- Et pour finir on va creer une vue qui est n'compos�e que des clients qui poss�dent un nombre d'articles sup�rieur au seuil d�fini.
    -- Ces clients sont consid�r�s comme Amis
    ...
    -- Pour tous les clients consid�r�s comme ami nous allons recommander les articles de sont client ami
    FOR mesClients IN (SELECT * FROM V_RcmdAmi) LOOP
        propositionArticle(mesClients.client1, mesClients.client2, dateDebut, dateFin);
    END LOOP;
    -- Pour finir on cr� la vue qui ne contient que les clients et les articles qu'on souhaite leur recommander
    -- Cette �tape n'est pas obligatiore mais elle nous permet de supprimer les doublons au niveau des recommandations
    -- De ce fait, un client aura une seule fois un article recommand�
